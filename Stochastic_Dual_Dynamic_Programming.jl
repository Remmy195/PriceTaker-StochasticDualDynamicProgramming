...
function simulate_and_extract_data(Ptaker, n_replications)
    simulations = SDDP.simulate(Ptaker, n_replications, Symbol[:e_stored, :e_charge, :e_discharge, :z_charge, :z_discharge, :e_sold, :e_pur])
    return simulations
end
...
function analyze_and_plot_simulations(simulations, Ptaker, h_label)
    ...
    results_filepath = "objective_values_and_confidence_interval_h$(h_label).txt"
...
for (h, bound) in zip(h_values, bounds)
    ...
    simulations = simulate_and_extract_data(Ptaker, n_replications)
